...
import com.glow.ui.activities.CartListActivity
import com.glow.utils.Constants
import com.glow.utils.GlideLoader
import kotlinx.android.synthetic.main.item_cart_layout.view.*

open class CartListAdapter (private val context: Context,
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        if(holder is MyViewHolder) {
            ...
            holder.itemView.ib_add_cart_item.setOnClickListener {
                val cartQuantity: Int = model.cart_quantity.toInt()
                if(cartQuantity < model.max_order_quantity.toInt()) {
                    val itemHashMap = HashMap<String, Any>()
                    itemHashMap[Constants.CART_QUANTITY] = (cartQuantity + 1).toString()
                    ...
                    if(context is CartListActivity) {
                        context.showErrorSnackBar(context.resources.getString(R.string.msg_for_available_stock, model.max_order_quantity), true)
                    }
                }
            }
        }
